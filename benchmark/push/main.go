...
func startPush(b, e int) {
	...
	for {
		for i := 0; i < len(bodys); i++ {
			resp, err := httpPost(fmt.Sprintf("http://%s/push/all?op=5&speed=10&plat=pc", os.Args[3]), "application/x-www-form-urlencoded", bytes.NewBuffer(bodys[i]))
			if err != nil {
			...
			resp.Body.Close()
			lg.Printf("post %s", string(bodys[i]))
